...
static unsigned char Moon_Engine_VSn[4] = { 2,2,11,0 };
...
_declspec(dllexport) extern void MoonProjectInit(const char* project_name, int x, int y, int width, int height, int fps, void (*ProjectSetting_1)())
{
...
_declspec(dllexport) extern void MoonProjectOver(void (*ProjectOverSetting)())
{
	while (1)
...
_declspec(dllexport) extern int MoonProjectPause(int mode, int (**function_1)(), int (*function_2)(), int (*function_3)())
{
...
_declspec(dllexport) extern void MoonProjectFunctionSwitch(MOON_MODULE_FUNCTION module, int (*function_2)())
{
...
_declspec(dllexport) extern int MoonProjectFindEntityAllNumber()
{
	int all_number = 0;
...
_declspec(dllexport) extern int MoonProjectGetMessage(MOON_MESSAGE_ALL* message, unsigned char* type, void(*Handle)(MOON_MESSAGE_ALL*, unsigned char*))
{
	//type的作用
	//防止随意操作标志导致消息处理紊乱
	*type = MOON_TRUE;
...
_declspec(dllexport) extern void MoonlogicMessageHandle(MOON_MESSAGE_ALL* message, unsigned char* type)
{
...
_declspec(dllexport) extern inline MOON_POINT2D MoonProjectWindowSize()
{
	MOON_POINT2D size;
