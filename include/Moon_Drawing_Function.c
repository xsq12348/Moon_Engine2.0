...
extern void MoonCoreDrawAreaUV(MOON_TEXTURE_VECTER* vertexs, unsigned int vertex_number, MOON_METADATA* metadata)
{
	if (!metadata->draw.image_goal || !metadata->draw.image.image_resources)
	{
		MoonPrompt("无效的纹理对象");
...
extern void MoonCoreDrawAreaPlgBit(MOON_TEXTURE_VECTER* vertexs, unsigned int vertex_number, MOON_METADATA* metadata)
{
	if (!metadata->draw.image_goal || !metadata->draw.image.image_resources)
	{
		MoonPrompt("无效的纹理对象");
