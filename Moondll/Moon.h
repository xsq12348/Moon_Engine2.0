...
作者:xsq12348 / XSQ / xsq1234(这三个都是别名)
...
请注意,本引擎/框架本质上是作为内核/类内核使用的,实际只提供最小功能,比如基础图元,工具函数,大部分情况仍需您自主实现
	...
	本项目地址 https://github.com/xsq12348/Moon_Engine2.0
...
[11] 消息式绘图{ 警告!!本条目为AI自动填充,如有错误,作者不对此负有主要责任 }
	...
	[11.5] 颜色说明
		...
		颜色格式为 0xAARRGGBB：(此处为AI错误,应该为ABGR格式)
...
* 2.2.13.1		2026.8.1	优化了单线程模式下的流程
* 2.2.14.0					添加了
*								MoonCtz
*							用于计算末尾0的数量
* 2.2.14.1		2026.8.9	修正了部分注释内容
* 2.2.14.2					修正了按鍵回调部分
*							保证其在单线程模式下符合单次触发的最初设想
*							而不是像多线程那样的短暂时间窗口下的短按和长按
*							是真正稳定的单次按下与长按
* 2.2.15.0					添加了
*								MoonStrStr
*							用于strstr的封装,但是返回索引而不是指针
* 2.2.16.0		2026.8.22	添加了
*								MoonStrReverse
*							用于反转字符串
* 2.2.16.1		2026.9.13	优化了部分代码
* 2.2.16.2					MOON_POINT2D的long int x, y
*							改为 int x, y
* 2.2.16.3					添加了
*								MoonCode_OFF/MoonCode_ON宏
*							用于关闭/开启代码块
* 2.2.16.4					添加了
*								MoonClamp
*							用于限制数值在指定范围内,与Range完全相同,但是语义上更清晰
* 2.2.16.5					修改了内置字体,现在更加美观
* 2.2.17.0+					添加了
*								MoonFindEntity_Id
*							用于直接查询实体
*
...
*/
_declspec(dllexport) extern void MoonProjectInit(const char* project_name, int x, int y, int width, int height, int fps,void (*ProjectSetting_1)());//创建项目
...
*/
_declspec(dllexport) extern void MoonProjectRun(void (*ProjectSetting_2)(), int(*ProjectLogic)(), int(*ProjectDrawing)());//运行项目
...
*/
_declspec(dllexport) extern void MoonProjectRun_Single(void (*ProjectSetting_2)(), int(*ProjectLogic)(), int(*ProjectDrawing)());//运行项目
...
*/
_declspec(dllexport) extern void MoonProjectOver(void (*ProjectOverSetting)());//结束项目
...
*/
_declspec(dllexport) extern int MoonProjectFindEntityAllNumber();//统计实体总数
...
*/
_declspec(dllexport) extern MOON_POINT2D MoonProjectWindowSize();
