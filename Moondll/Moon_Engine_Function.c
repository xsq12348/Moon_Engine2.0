...
_declspec(dllexport) extern void MoonProjectFunctionSwitch(char module, int (*function_2)())
{
...
_declspec(dllexport) extern int MoonProjectGetMessage(MOON_MESSAGE_ALL* message, _Bool* type, void(*Handle)(MOON_MESSAGE_ALL*, _Bool*))
{
	//type的作用
	//防止随意操作标志导致消息处理紊乱
	*type = MOON_TRUE;
...
_declspec(dllexport) extern void MoonlogicMessageHandle(MOON_MESSAGE_ALL* message, _Bool* type)
{
